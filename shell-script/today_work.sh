out_file_path="$(pwd)/today_log.txt"
...
echo "---" >"$out_file_path"
echo "**($(date -d last-day '+%Y-%m-%d %H:%M:%S'), $(date '+%Y-%m-%d %H:%M:%S'))工作内容统计:**" >>"$out_file_path"

for workspace_path in "${workspace_paths[@]}"; do
	{
		cd "$workspace_path"
		git_log_out=$(git log --pretty=format:"	- \`%cd\` %cn%s" --after="yesterday" --date=format:"%a" --author="xing" --no-merges | grep -v "ignored")
